...
class Client
{
	 ...
	 */
	public function __construct($transport=null, $port=2480, $database=null)
	{
		...
		try {
			if ($transport === null) {
				...
				$transport = new Transport\Curl($transport, $port, $database);
			}
		} catch (Exception $e) {
			if ($transport === null) {
				$transport = new Transport\Stream();
			} else if (is_string($transport)) {
				$transport = new Transport\Stream($transport, $port, $database);
			}
		}
